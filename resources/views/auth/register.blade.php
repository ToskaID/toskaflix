    ...
    <form class="form" action="{{ route('register') }}" method="POST" >
        ...
        <div class="mb-3 f-password">
            <input type="password" name="password"
                class="form-control form-password @error('password') is-invalid @enderror" id="InputPassword" autocomplete="new-password" required>
    ...
    <script>
        document.querySelectorAll('.toggle-password').forEach(toggle => {
            toggle.addEventListener('click', function() {
                // Mencari input di dalam container yang sama (mb-3)
                const input = this.closest('.mb-3').querySelector('input');
